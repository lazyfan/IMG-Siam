import os
from utils.matting_utils import *
 
def sbbm(frame, segments, labels, 
         delta=0.5, eta=0.8, R=20):
    ...
    for i, sp_no in enumerate(superpixel_inds[labels]):
        ...
        Z = samples[:, :, np.newaxis, :] - pixel_values[np.newaxis, np.newaxis, :] # shape(n_training, N, npix, D)
        Z *= Z
...
def perform_SBBM_segmentation(image, bbox, delta=0.5, eta=0.5, crop_ratio=2, return_crop_region=False):
    """
    ...
    # classify with SBBM
    pred = sbbm(c_image, segments, sp_labels, 
                delta=delta, eta=eta)
...
if __name__ == "__main__":
    ...
    image = Image.open(CURRENT_PATH + '/images/crossing_00000066.jpg')
    image = np.array(image)
    bbox = np.array([495.64, 261.45, 543.01, 264.25, 
                     532.31, 445.24, 484.94, 442.44])

...
class Tracker(object):
  ...
  def track(self, sess, first_bbox, frames, matting_method, logdir='/tmp'):
    ...
    frame2crop_scale = self.siamese_model.initialize(sess, input_feed, matting_method, logdir)
    ...
    logging.info('The video has {} frames, tracking time is {:.2f}, total time is {:.2f}'.format(len(frames),
                                                                                            ...
                                                                                            end_time-start_time))
    logging.info('The tracking speed is %.2f FPS'%(len(frames)/(end_time-start_time)))
